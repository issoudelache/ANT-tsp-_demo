...
Version avec support multi-cœur parallèle.
"""
import argparse
import sys
import os
...
Exemples d'utilisation:
  python run_benchmarks.py                           # Benchmarks complets séquentiels
  python run_benchmarks.py --quick                   # Tests rapides
  python run_benchmarks.py --parallel                # Benchmarks parallèles ({n_cores} cœurs)
  python run_benchmarks.py --parallel --jobs 4       # Parallèle avec 4 cœurs
  ...
  python run_benchmarks.py --append                  # Ajouter aux résultats existants
    ...
    parser.add_argument(
        '--parallel', '-p',
        action='store_true',
        help=f'Mode parallèle : utilise tous les cœurs disponibles ({n_cores} sur cette machine)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help=f'Nombre de processus parallèles (défaut: tous les cœurs = {n_cores})'
    )
    ...
    n_processes = args.jobs if args.jobs else n_cores
    ...
    print(f"Exécution: {'Parallèle' if args.parallel else 'Séquentielle'}")
    if args.parallel:
        print(f"Cœurs utilisés: {n_processes}/{n_cores}")
    ...
    try:
        df = run_and_save_benchmarks(
            ...
            n_processes=n_processes if args.parallel else None
        )
